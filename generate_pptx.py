...
def slide_02_global_cases(prs):
    ...
    add_insight_line(slide, "이 기업들은 AI를 '도입'한 것이 아니라, 인력 비용 구조 자체를 AI 기반으로 전환했다")
...
def slide_05_disruption(prs):
    ...
    add_insight_line(slide, "업무 방식을 바꾸는 것이 경쟁력을 지키는 유일한 방법이다")
...
def slide_06_current_state(prs):
    ...
    add_insight_line(slide, "현재 구조에서는 매출이 늘면 사람도 늘어야 한다")
...
def slide_07_before_after(prs):
    ...
    add_insight_line(slide, "글로벌 기업 대비 보수적인 목표이다 — 구체적인 실행 방법을 지금부터 설명한다")
...
def slide_08_two_layers(prs):
    ...
    add_insight_line(slide, "CAIO의 Top-down 구축과 각 부서의 Bottom-up 활용이 함께 이루어져야 한다")
...
def slide_12_roadmap_overview(prs):
    ...
    add_insight_line(slide, "오늘 승인하시면 Layer 1은 내일부터 착수, Layer 2 제도는 즉시 시행한다")
...
def slide_13_tech_approach(prs):
    ...
    add_insight_line(slide, "검증된 아키텍처 위에 빠르게 구축하고, 결과를 바로 확인한다")
...
def slide_14_candidate_map(prs):
    ...
    add_insight_line(slide, "8개 후보 중 비용 절감 가시성과 독립 구축 가능성이 높은 3개를 선별했다")
...
def slide_15_comparison(prs):
    ...
    add_insight_line(slide, "최종 결정은 경영진의 판단 사항입니다")
...
def slide_17_execution_plan(prs):
    ...
    add_insight_line(slide, "먼저 완성하고 결과를 보여준 뒤, 쓰면서 고도화한다")
...
def slide_18_success_metrics(prs):
    ...
    add_insight_line(slide, "사전에 방어선을 설계하고, 결과를 빠르게 보여주는 것이 최선의 대응이다")
...
def slide_19_why_policy(prs):
    ...
    add_insight_line(slide, "기술 도입이 아니라 제도 설계가 Layer 2의 본질이다")
...
def slide_20_policy1(prs):
    ...
    add_insight_line(slide, "사람을 더 뽑는 것이 아니라, AI를 먼저 활용하는 것이 기본값이 된다")
...
def slide_21_policy2(prs):
    ...
    add_insight_line(slide, "AI 활용이 선택이 아닌 필수가 된다")
...
def slide_22_policy3(prs):
    ...
    add_insight_line(slide, "같은 일을 하는 팀 중 하나가 AI로 앞서면, 나머지는 따라오거나 평가에서 뒤처진다")
...
def slide_23_policy4(prs):
    ...
    add_insight_line(slide, "챔피언이 부서의 변화를 이끌고, Layer 1과의 연결점이 된다")
...
def slide_24_integrated_timeline(prs):
    ...
    add_insight_line(slide, "두 Layer가 합류하면서 상호 강화한다")
...
def slide_25_expected_effects(prs):
    ...
    add_insight_line(slide, "인당 매출 3억 원은 달성 가능한 목표이다")
...
def slide_26_caio_role(prs):
    ...
    add_insight_line(slide, "CAIO가 모든 것을 직접 하는 것이 아니라, 구조를 설계하고 조직이 성장하게 한다")
...
def slide_27_risk_early(prs):
    ...
    _risk_table_slide(prs,
        ...
        "불신과 거부 — 결과를 빠르게 보여주는 것이 최선의 대응이다")
...
def slide_28_risk_mid(prs):
    ...
    _risk_table_slide(prs,
        ...
        "형식화와 양극화 — 동일 직무 경쟁 평가제가 두 가지 모두에 대응한다")
...
def slide_29_risk_long(prs):
    ...
    _risk_table_slide(prs,
        ...
        "사람 의존과 안주 — Layer 2의 성과를 Layer 1에 편입하는 구조가 방지한다")
